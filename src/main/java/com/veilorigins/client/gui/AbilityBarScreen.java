...
public class AbilityBarScreen extends Screen {
    ...
    public void tick() {
        ...
        if (isMousePressed && !wasMousePressed) {
            if (hoveredAbility >= 0 && hoveredAbility < abilities.size()) {
        ...
        // Number keys 1-9
        for (int i = 0; i < 9; i++) {
            if (org.lwjgl.glfw.GLFW.glfwGetKey(windowHandle, org.lwjgl.glfw.GLFW.GLFW_KEY_1 + i) == org.lwjgl.glfw.GLFW.GLFW_PRESS) {
                if (i < abilities.size()) {
                    activateAbility(i);
                    return;
                }
        ...
        // R for ability 1
        if (org.lwjgl.glfw.GLFW.glfwGetKey(windowHandle, org.lwjgl.glfw.GLFW.GLFW_KEY_R) == org.lwjgl.glfw.GLFW.GLFW_PRESS && abilities.size() > 0) {
            activateAbility(0);
            return;
        }
        
        // V for ability 2
        if (org.lwjgl.glfw.GLFW.glfwGetKey(windowHandle, org.lwjgl.glfw.GLFW.GLFW_KEY_V) == org.lwjgl.glfw.GLFW.GLFW_PRESS && abilities.size() > 1) {
            activateAbility(1);
            return;
        }
        
        // T for skill tree
        if (org.lwjgl.glfw.GLFW.glfwGetKey(windowHandle, org.lwjgl.glfw.GLFW.GLFW_KEY_T) == org.lwjgl.glfw.GLFW.GLFW_PRESS) {
    ...
    private void activateAbility(int index) {
        ...
        if (ability.isOnCooldown()) {
            Minecraft.getInstance().player.displayClientMessage(
                Component.literal("Ability on cooldown: " + formatCooldown(ability.getCooldown()))
